...
describe('Add source controller', () => {
    ...
    beforeEach(() => {
        req = {body: {title: 'Source title 1', author: 'author 1', format: 'print'}, user: {userId: 1}} as Request;
    });
...
describe('Get source lookup controller', () => {
    ...
    test('Return 200 when source lookup data successfully fetched', async () => {
        ...
        // ARRANGE
        const mockSourceLookup = [{id: 1, title: 'Source 1'}, {id: 2, title: 'Source 2'}];
        (mockedDb.select as jest.Mock).mockReturnValue({
            from: jest.fn().mockReturnValue({
...
describe('Update source controller', () => {
    ...
    beforeEach(() => {
        req = {body: {title: 'New Source Title'}, user: {userId: 1}, params: {id: '1'}} as unknown as Request;
    });
