...
export const addSheet = async (req: Request, res: Response, next: NextFunction) => {
	try {
		...
		const { title, key, composer, sourceId, levelId, genreId, examPiece } = req.body;
...
export const getSheet = async (req: Request, res: Response, next: NextFunction) => {
	try {
		...
		const keyFilter: string | undefined =
			keyQuery && keyQuery !== 'all' ? (keyQuery as string) : undefined;
		const levelFilter: number | undefined =
			levelQuery && levelQuery !== 'all' ? parseInt(levelQuery as string) : undefined;
		const genreFilter: number | undefined =
			genreQuery && genreQuery !== 'all' ? parseInt(genreQuery as string) : undefined;
...
export const updateSheet = async (req: Request, res: Response, next: NextFunction) => {
	try {
		...
		const updatedSheet = await db
			...
			.where(and(eq(sheet.id, parseInt(id)), eq(sheet.userId, userId)))
			.returning();
