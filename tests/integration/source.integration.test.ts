...
import { sourceResponse } from '../../src/types/source';
...
describe('source API', () => {
	...
	describe('POST /source', () => {
		it('should return 401 when no token is provided', async () => {
			const res = await request(app).post('/api/source').send({ title: 'Test source' });
	...
	describe('GET /source', () => {
		...
		it('should return 200 and return source data', async () => {
			const res = await request(app).get('/api/source').set('Authorization', `Bearer ${token}`);

			const createdSource = res.body.data.find((source: sourceResponse) => source.id === sourceId);
	...
	describe('GET /source/lookup', () => {
		it('should return 401 when no token is provided', async () => {
			const res = await request(app).get('/api/source/lookup');
	...
	describe('DELETE /source', () => {
		it('should return 401 when no token is provided', async () => {
			const res = await request(app).delete(`/api/source/${sourceId}`);
