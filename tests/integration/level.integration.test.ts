...
import { levelResponse } from '../../src/types/level';
...
describe('LEVEL API', () => {
	...
	describe('POST /level', () => {
		it('should return 401 when no token is provided', async () => {
			const res = await request(app).post('/api/level').send({ name: 'Test level' });
	...
	describe('GET /level', () => {
		...
		it('should return 200 and return level data', async () => {
			const res = await request(app).get('/api/level').set('Authorization', `Bearer ${token}`);

			const createdlevel = res.body.data.find((level: levelResponse) => level.id === levelId);
	...
	describe('PUT /level', () => {
		it('should return 401 when no token is provided', async () => {
			const res = await request(app).put(`/api/level/${levelId}`).send({ name: 'Updated level' });
	...
	describe('DELETE /level', () => {
		it('should return 401 when no token is provided', async () => {
			const res = await request(app).delete(`/api/level/${levelId}`);
